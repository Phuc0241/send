...
// File input listeners (both files and folders)
const fileInputFiles = document.getElementById('fileInputFiles');
const fileInputFolders = document.getElementById('fileInputFolders');
...
fileInputFolders.addEventListener('change', (e) => {
    const files = Array.from(e.target.files);
    handleFiles(files);
});
...
function handleFiles(files) {
    ...
    // Warn if file is too large (>5GB)
    const maxSafeSize = 5 * 1024 * 1024 * 1024; // 5GB
    if (totalSize > maxSafeSize) {
        const sizeGB = (totalSize / (1024 * 1024 * 1024)).toFixed(2);
        if (!confirm(`⚠️ Warning: Total size is ${sizeGB}GB.\n\nLarge transfers may fail due to memory limits.\n\nRecommendation: Send files in smaller batches (< 5GB each).\n\nContinue anyway?`)) {
            return;
        }
    }
...
async function downloadSingleFile(transferId, manifest) {
    ...
    try {
        ...
        console.log('PyWebView detected:', !!window.pywebview);
        if (window.pywebview && window.pywebview.api) {
            ...
            reader.onloadend = async function () {
                try {
                    ...
                    console.log('Calling save_file API...');
