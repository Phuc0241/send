...
async function downloadSingleFile(transferId, manifest) {
    ...
    for (let i = 0; i < totalChunks; i++) {
        ...
        while (!success && retries < maxRetries) {
            try {
                const response = await fetch(`${RELAY_URL}/transfer/${transferId}/chunk/${i}`);
...
async function downloadFolder(transferId, manifest) {
    ...
    for (const fileInfo of files) {
        ...
        // Download all chunks for this file
        for (let i = 0; i < fileInfo.totalChunks; i++) {
            ...
            while (!success && retries < maxRetries) {
                try {
                    const response = await fetch(`${RELAY_URL}/transfer/${transferId}/chunk/${globalChunkIndex}`);
        ...
        // Add file to ZIP with full path structure
    ...
    const zipBlob = await zip.generateAsync({
        type: 'blob',
        compression: 'STORE' // No compression since files are already compressed
    });
